  ...
  return async (dispatch) => {
    ...
    const fetchCart = async () => {
      const response = await fetch(
        "https://italia-db1ec-default-rtdb.europe-west1.firebasedatabase.app/cart.json"
      );
...
export const sendData = (cart, message) => {
  return async (dispatch) => {
    ...
    try {
      dispatch(
        uiActions.showNotification({ message: message, type: "CHANGE" })
      );
      await putData();
    } catch (err) {
      dispatch(
        uiActions.showNotification(
          "Something went wrong with adding item to cart"
        )
      );
      console.log(err);
    }
